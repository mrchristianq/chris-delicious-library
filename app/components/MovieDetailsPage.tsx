...
export function MovieDetailsPage({ item, isMobileLayout, onBack, onEdit, getDisplayCoverUrl, onPaletteChange }: MovieDetailsPageProps) {
  ...
  const statusColor = (() => {
    ...
    if (s==="watched"||s==="completed") return {bg:"rgba(34,197,94,0.18)",border:"rgba(134,239,172,0.5)",text:"#8df5ad"};
    if (s==="started"||s==="watching") return {bg:"rgba(234,179,8,0.18)",border:"rgba(253,224,71,0.38)",text:"#fde047"};
    if (s==="abandoned"||s==="dropped") return {bg:"rgba(249,115,22,0.18)",border:"rgba(253,186,116,0.38)",text:"#fdba74"};
    return { bg: palette.chip, border: palette.surfaceBorder, text: palette.text };
  })();
          ...
          <div style={{ position: "absolute", top: 14, right: 16, display: "flex", gap: 8, zIndex: 3 }}>
            {onEdit ? (
              <button
                ...
                onClick={() => onEdit(item)}
                style={{
                  borderRadius: 999, padding: "7px 15px", fontSize: 13, lineHeight: 1, fontWeight: 750,
            ...
            {watchStatus ? (
              <span style={{
                borderRadius: 999, padding: "7px 13px", fontSize: 13, lineHeight: 1, fontWeight: 800,
                background: statusColor.bg, border: `1px solid ${statusColor.border}`,
                color: statusColor.text, backdropFilter: "blur(8px)",
              }}>
